...
export const getMarketStats = cache(async (): Promise<MarketStats> => {
  ...
  const cachedStats = await redis.get(REDIS_KEYS.MARKET_STATS)
  if (cachedStats) {
    try {
  ...
  try {
    await redis.set(REDIS_KEYS.MARKET_STATS, JSON.stringify(stats), { ex: 300 })
  } catch (error) {
...
export const getTrendingCoins = cache(async (): Promise<TrendingCoin[]> => {
  const redis = getRedisClient();
  const cachedCoins = await redis.get(REDIS_KEYS.TRENDING_COINS)
  if (cachedCoins) {
    try {
  ...
  try {
    await redis.set(REDIS_KEYS.TRENDING_COINS, JSON.stringify(coins), { ex: 60 })
  } catch (error) {
...
export const getFeatureStats = cache(async (): Promise<FeatureStats> => {
  const redis = getRedisClient();
  const cachedStats = await redis.get(REDIS_KEYS.FEATURE_STATS)
  if (cachedStats) {
    try {
  ...
  try {
    await redis.set(REDIS_KEYS.FEATURE_STATS, JSON.stringify(stats), { ex: 3600 })
  } catch (error) {
